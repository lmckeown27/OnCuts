...
export default function AdminHeader({ title }: Props) {
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <div 
          className={`fixed inset-0 min-h-[100dvh] flex items-center justify-center z-50 p-4 transition-all duration-200 ease-out ${
            isModalVisible ? 'bg-black/50' : 'bg-black/0'
          }`}
          onClick={closeModal}
        >
          <div 
            className={`bg-white rounded-2xl shadow-2xl max-w-lg w-full max-h-[85dvh] sm:max-h-[90vh] overflow-y-auto transition-all duration-200 ease-out ${
              isModalVisible 
