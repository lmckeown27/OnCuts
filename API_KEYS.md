...
**Native — Sign in with Apple (`POST /api/v1/auth/apple`)**: Apple only provides `ASAuthorizationAppleIDCredential.email` and `fullName` on the **first** authorization for your app. The client must POST them with `identityToken` every time that data is non-nil (typically the first sign-in). The API accepts `email` / `userEmail`, and names as `firstName`/`lastName`, **`givenName`/`familyName`**, or nested **`fullName`**: `{ givenName, familyName }` (see `auth.controller` helpers `pickAppleCredentialEmail`, `readApplePersonNameFromBody`). New Apple accounts get `has_platform_password = false` until the user calls **`PUT /api/v1/users/me/set-initial-password`** (Bearer JWT, body `{ "newPassword": "..." }`, min 8 chars). Auth responses include **`needsPlatformPassword`** when true. Apply migration **`027_has_platform_password.sql`** on Postgres before deploying code that reads that column. **If Apple sign-in returns 503** (`APPLE_SCHEMA_MISSING`), apply **026** and **027** on the API database, then restart PM2: from `backend/`, `npm run migrate:sql:apple` (or `npm run migrate:sql -- 026 027`).
