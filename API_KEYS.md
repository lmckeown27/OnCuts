...
| `APPLE_CLIENT_ID` | Public ID (still server-trusted config) | iOS bundle ID or Sign in with Apple **Services ID**; must match the `aud` claim when verifying Apple `identityToken` on `POST /api/v1/auth/apple` (`apple-auth.service`, `auth.controller`). |
