-- Path B on Prisma-managed `users` (camelCase "walletAddress" already holds legacy hex).
...
COMMENT ON COLUMN users.sui_address IS 'Sui payout / zkLogin address (Path B)';
